...
`flake.nix` 构建 `homeConfigurations.fym`，并通过 `extraSpecialArgs` 注入 `localLib`。  
...
| [config/gui/fuzzel.nix](config/gui/fuzzel.nix) | fuzzel 启用与 genericLinux 包处理 |
...
3. 非必要不要修改 [config/common/home-manager.nix](config/common/home-manager.nix) 中的 `home.stateVersion`。
4. Nix 文件遵循 [.editorconfig](.editorconfig) 的 2 空格缩进。

### 4.3 快速定位

1. 想改终端行为：看 [config/gui/terminal.nix](config/gui/terminal.nix)
2. 想改 Niri 快捷键：看 [config/gui/niri/settings/binds.nix](config/gui/niri/settings/binds.nix)
3. 想改输入法：看 [config/gui/fcitx5/default.nix](config/gui/fcitx5/default.nix)
4. 想改开发工具包：看 [config/common/misc/packages/default.nix](config/common/misc/packages/default.nix)
5. 想加新模块：先看 [config/default.nix](config/default.nix) 与 [lib/ls.nix](lib/ls.nix)
